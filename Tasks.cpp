...
void onAlerta()
{
  stopAllTasks();
  estadoActual = ALERTA;
  if (intentosAlerta <3) {intentosAlerta++; }
...
void funcLedRGB()
{
  ...
  switch (estadoActual)
  {
    case INICIO: digitalWrite(LedVerde, ledState ? LOW : HIGH);
                 TaskLed.SetIntervalMillis(ledState ? 200 : 200); break;

    case ALERTA:  digitalWrite(LedAzul, ledState ? LOW : HIGH);
                  TaskLed.SetIntervalMillis(ledState ? 500 : 900); break;

    case ALARMA: digitalWrite(LedRojo, ledState ? LOW : HIGH);
                 TaskLed.SetIntervalMillis(ledState ? 100 : 300); break;
